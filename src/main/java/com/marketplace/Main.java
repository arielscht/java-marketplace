...
public class Main {
    public static void main(String[] args) {
        ...
        HashMap<String, Object> result = chatController.sendMessage("Oi tudo bem?", chat);

        if(result.get("chat") != null)
            System.out.println("Tudo ok!");

        Iterator<Message> iterator = currentUser.getChats().findById(chat.getId()).getMessages().iterator();
